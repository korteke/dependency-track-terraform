...
resource "aws_kms_key" "encryption_key" {
  description         = "DT Encryption Key"
  is_enabled          = true
  enable_key_rotation = true

  tags = merge({
    Name = "${var.name_prefix}-dt-kms-key"
  }, var.tags)
}
...
resource "aws_db_subnet_group" "aurora_db_subnet_group" {
  name       = "${var.name_prefix}-dt-aurora-db-subnet-group"
...
resource "aws_rds_cluster" "aurora_db" {
  ...
  # Cluster
  cluster_identifier     = "${var.name_prefix}-dt-aurora-db"
  ...
  engine_version  = local.dt_db_engine_version
  database_name   = local.dt_db_name
  ...
  tags = merge({
    Name = "${var.name_prefix}-dt-aurora-db"
  }, var.tags)
}
...
resource "aws_rds_cluster_instance" "aurora_db_cluster_instances" {
  ...
  engine               = "aurora-postgresql"
  engine_version       = local.dt_db_engine_version
  instance_class       = local.dt_db_instance_size
  publicly_accessible  = false
  tags = merge({
    Name = "${var.name_prefix}-dt-aurora-db-cluster-instances-${count.index}"
  }, var.tags)
}
